...
namespace SCN.Tutorial 
{
	public class TutorialManager : MonoBehaviour
	{
		...
		{
			isFirstTime = true;
			StartPointerLoop(pos, isRight);
		}
		void StartPointerLoop(Vector3 pos, bool isRight = true)
		{
			WaitUserNoReact(() =>
			{
		...
		{
			isFirstTime = true;
			StartPointerLoop(transform, isRight);
		}
		void StartPointerLoop(Transform transform, bool isRight = true) 
		{
			WaitUserNoReact(() =>
			{
		...
		{
			isFirstTime = true;
			StartPointerLoop(startPos, endPos, isRight);
		}
		void StartPointerLoop(Vector3 startPos, Vector3 endPos, bool isRight = true)
		{
			WaitUserNoReact(() =>
			{
		...
		void MovePointer(Vector3 startPos, Vector3 endPos)
		{
			_ = DOTweenManager.Instance.TweenMoveVelocity(pointerTrans, startPos, endPos, 
				pointerVelocity, false).SetEase(Ease.Linear).SetLoops(-1);
		}
		...
		{
			var timer = 0f;
			var waitTime = isFirstTime ? delayStartTime : noReactTime;
